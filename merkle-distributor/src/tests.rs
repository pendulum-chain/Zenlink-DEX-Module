...
fn one_level_merkel_tree_proof_should_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(MdPallet::add_to_create_whitelist(RawOrigin::Root.into(), ALICE,));
...
fn claim_towice_should_not_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			MdPallet::claim(RawOrigin::Signed(BOB).into(), 0, 0, BOB, 10_000_000 * UNIT, proof),
...
fn claim_use_worng_index_should_not_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			MdPallet::claim(RawOrigin::Signed(ALICE).into(), 0, 2, BOB, 10_000_000 * UNIT, proof),
			Error::<Runtime>::MerkleVerifyFailed
		);
...
fn add_to_whitelist_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(MdPallet::add_to_create_whitelist(RawOrigin::Root.into(), BOB,));
...
fn remove_from_whitelist_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(MdPallet::remove_from_create_whitelist(RawOrigin::Root.into(), ALICE,));
